...
def filt(string, start, end):
    ...
    if start != '': i = string.find(start) + len(start)
    else: i = 0
    j = string[i:].find(end)
    return string[i : i + j]
...
class AutohomeAllPromotionSpider(BaseSpider):
    ...
    def parsePromotion(self, response):
        ...
        item['datefrom'] = tmp[0].replace(u' ', '')
        item['dateto'] = tmp[1].replace(u' ', '')
        ...
        if tables:
            for table in tables:
                ...
                t['oprice'] = table.xpath('td[2]/text()').extract()[0]
                t['bprice'] = table.xpath('td[4]/text()').extract()[0]
                t['dprice'] = table.xpath('td[5]/text()').extract()[0]
                ts.append(t)
        item['prices'] = ts
        item['brand_id'] = 2
